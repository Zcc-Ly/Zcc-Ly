           ...
           <div id="navbar" class="collapse navbar-collapse">
            <ul class="nav navbar-nav">
              <li ><a href="allst_Stu.jsp">所有社团</a></li>
                <li class="active"><a href="toStust">我的社团</a></li>
	...
	$.each(club,function(index,item){
		...
		*/
		...
		var delBtn =  $("<button></button>").addClass("btn btn-danger btn-sm delete_btn")
						.append($("<span></span>").addClass("glyphicon glyphicon-trash")).append("退出社团");
		
		//为删除按钮添加一个自定义的属性来表示当前删除的社团id
		delBtn.attr("del-id",item.stno);
		
		var btnTd = $("<td></td>").append(ckBtn).append(" ").append(delBtn);
...
$(document).on("click",".delete_btn",function(){
	...
	var stid = $(this).attr("del-id");
	//alert($(this).parents("tr").find("td:eq(1)").text());
	if(confirm("确认退出【"+clubName+"】吗？")){
		//确认，发送ajax请求删除即可
		$.ajax({
			url:"${APP_PATH}/quitclub/"+${stu.id},
			data:{
				"sId":${stu.id},
				"stId":stid
			},
